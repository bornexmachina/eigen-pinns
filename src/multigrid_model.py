...
class MultigridGNN:
    ...
    def __init__(self, model_type='simple'):
        ...
        if self.model_type not in ['simple', 'spectral']:
            raise ValueError(f"model_type must be 'simple' or 'spectral', got '{model_type}'")

    def train_multiresolution(
        self, 
        X_list, K_list, M_list, U_init_list, P_list, edge_index_list,
        epochs, lr, corr_scale, 
        ...
        node_offset_list = self._compute_node_offsets(X_list)
        
        # Step 1: Apply coarse grid correction and compute eigenvalues
        U_CGC_list, lambda_list = self._initialize_cgc_hierarchy(
            U_init_list, K_list, M_list, P_list
        )
        
        # Step 2: Normalize base eigenvectors with M-norm
        U_CGC_normalized_list = self._normalize_eigenvectors(U_CGC_list, M_list)
        U_all_CGC_norm = torch.cat(U_CGC_normalized_list, dim=0)
        
        # Step 3: Build physics-informed node features
        x_feats_all, edge_index_all_tensor, A_norm_all = self._build_features(
            X_list, U_CGC_normalized_list, lambda_list, edge_index_list, K_list, M_list
        )
        
        # Step 4: Initialize model and optimizer
        self._initialize_model(x_feats_all.shape[1], n_modes, hidden_layers, dropout)
        optimizer, scheduler = self._create_optimizer(lr, weight_decay)
        
        # Step 5: Training loop
        self._training_loop(
            x_feats_all, edge_index_all_tensor, A_norm_all, U_all_CGC_norm,
            K_list, M_list, lambda_list, node_offset_list,
            optimizer, scheduler, epochs, corr_scale, 
            w_res, w_orth, w_proj, w_trace, w_order, w_eigen,
            grad_clip, log_every, n_modes
        )
        
        # Step 6: Generate final predictions
        return self._generate_final_predictions(
            x_feats_all, edge_index_all_tensor, A_norm_all, U_all_CGC_norm,
            U_CGC_normalized_list, M_list, corr_scale
        )
    ...
    def _training_loop(
        ...
        for epoch in range(epochs):
            ...
            loss_res, loss_orth, lambda_pred = self._compute_residual_ortho_loss(
                U_pred, K_list, M_list, node_offsets, w_res, w_orth, n_modes
            )
            loss_proj, loss_trace, loss_order, loss_eigen = self._compute_eigenvalue_losses(
                M_list, lambda_list[0], lambda_pred, w_proj, w_trace, w_order, w_eigen
            )
    ...
    def _generate_final_predictions(self, x_feats, edge_index, A_norm, U_base, U_normalized_list, M_list, corr_scale):
